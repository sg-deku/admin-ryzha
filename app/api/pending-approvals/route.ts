...
export async function POST(request: NextRequest) {
  ...
  const { organizationId, action, defaultPlanId } = body
      ...
      if (defaultPlan) {
        await tx.license.upsert({
          ...
          create: {
            ...
            maxUsers: (defaultPlan.features as any)?.maxUsers ?? 5,
            maxApiCalls: (defaultPlan.features as any)?.maxApiCalls ?? 10000,
            maxAiTokens: (defaultPlan.features as any)?.aiTokens ?? 50000,
          },
          update: {},
        })
      }
